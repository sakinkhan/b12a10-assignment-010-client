...
const Register = () => {
  ...
  // ✅ If user is already logged in, send them home (no "already logged in" UI)
  useEffect(() => {
    if (user) {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // ✅ Always go home after successful register
      navigate("/", { replace: true });
    } catch (error) {
  ...
  const handleRegisterUsingGoogle = async () => {
    try {
      ...
      // Optional: if you still want to respect "from" when coming from a protected route
  ...
  // ✅ While redirecting logged-in users, render nothing (or a loader if you want)
  if (user) return null;
    ...
    <div className="min-h-screen bg-blue-50 dark:bg-[#0b0b0b] transition-colors duration-300 flex items-center justify-center px-10">
      ...
      <div className="card w-full max-w-md bg-white dark:bg-[#1a1a1a] shadow-2xl py-8 px-6 transition-colors duration-300">
        ...
        >
          ...
          <button
            type="submit"
            className="btn w-full border-0 mt-4 text-[16px] font-primary bg-[#108251] hover:bg-green-600 text-white dark:bg-[#4ade80] dark:text-black dark:hover:bg-[#22c55e] transition-all duration-200 hover:scale-[1.02] rounded-full"
          >
            Sign Up
          </button>
          ...
          <div className="border-t border-gray-300 dark:border-gray-700 pt-5">
            <button
              ...
              type="button"
              className="btn w-full flex items-center justify-center gap-2 bg-white text-black border-gray-300 hover:bg-gray-100 dark:bg-[#2a2a2a] dark:text-gray-100 dark:border-gray-700 dark:hover:bg-[#3a3a3a] transition-all duration-200 rounded-full"
            >
              <svg
